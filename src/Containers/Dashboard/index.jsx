...
class Dashboard extends Component {
  ...
  handleReminder = (day, currentDay) => {
    ...
    if (day > 0) {
      ...
      this.props.updateModal({
        payload: {
          status: true,
          element: <Reminder currentDate={currentDay} />
        }
      });
    }
  };
  ...
  handleEdit = data => {
    ...
    this.props.updateModal({
      payload: { status: true, element: <Reminder isEdition /> }
    });
  };
  ...
  currentDays = () => {
    ...
    for (let i = 0; i < this.firstDayOfMonth(); i++) {
      blankDays.push(
        <Day key={i * 10} day="" handleReminder={this.handleReminder} />
      );
    }
    for (let d = 1; d <= date.daysInMonth(); d++) {
      ...
      const reminders =
        currentDay in reminderReducer ? reminderReducer[currentDay] : [];
      monthDays.push(
        <Day
          key={d}
          day={d}
      ...
      <Grid container spacing={0}>
        <Paper className={classes.root}>
          <Table className={classes.table}>
            ...
            <TableBody>
              <TableRow>Product Example</TableRow>
            </TableBody>
          </Table>
        </Paper>
        <Modal />
...
const mS = ({ reminderReducer, reminderFormReducer }) => ({
  reminderReducer,
  reminderFormReducer
});
